...
require("../main/main.class.php");
require("creation.class.php");
...
function PlayGameEven(){
  ...
  $match = $obj->LoadOne('playgame',array('matchID'=>$data['matchID'],'memberID' => $data['memberID'],'gametype' => $data['gametype'],'gameeven' => $data['gameeven']));
  if($match){
    $result['success'] = 'PLAY';
    echo json_encode($result);
    exit;
...
//print_r($rowm);
  ...
  if($opoint  >= 2000){
    $result['success'] = 'POINTLIMIT';
    echo json_encode($result);
    exit;
  }
  
  $tpoint = $data['point']+$opoint;
  if($tpoint  >= 2000){
    $ttpoint=(2000-$opoint);
  }
    ...
    $sql2 = " UPDATE members SET score = score-'".$ttpoint."' WHERE code = '".$data['memberID']."' ";
    mysql_query($sql2);




	$sql2 = " UPDATE ".$table." SET point = point+'".$ttpoint."' WHERE code = '".$_GET['ID']."' ";
    mysql_query($sql2);
  $result['success'] = 'COMPLETE';
  //$result = $obj->Add($table, $data);  
  //$obj->Log('ADD', $table, $result['code'], $user);
...
$datestop = strtotime('+1 day', strtotime(date('Y-m-d 11:59:59')));
  ...
  playgame
  WHERE
  memberID = '".$data['memberID']."' AND gametype='rank' AND unix_timestamp(date_create) > '".$datestart."'
...
switch($_REQUEST["mode"]){
  ...
  case "PlayGameEven" : PlayGameEven(); break;
  case "PlayGame" : PlayGame(); break;
  case "PlayGameA" : PlayGameA(); break;
