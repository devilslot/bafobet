							...
							'<div class="plupload_filelist_footer">' +
								'<div class="plupload_file_name">' +
									'<div class="plupload_buttons">' +
										'<a href="#" class="plupload_button plupload_add btn btn-lg btn-primary"><span class="fa fa-upload"></span> ' + _('Add files') + '</a> ' +
									'</div>' +
			...
			this.each(function() {
				...
				function updateList() {
					...
					if (!uploader.files.length && uploader.features.dragdrop && uploader.settings.dragdrop) {
						$('#' + id + '_filelist').append('<li class="plupload_droptext"><h3 style="color:#666666 !important;">' + _("Drag files here.") + '</h3></li>');
					}
				}
				...
				uploader.bind("Error", function(up, err) {
					...
					if (file) {
						...
						if (err.code == plupload.FILE_SIZE_ERROR) {
							alert('Error : อัพโหลดขนาดไฟล์ใหญ่ได้ไม่เกิน ' + settings.max_file_size + '\nหากต้องการพื้นที่เพิ่มเติมกรุณาสมัครสมาชิก');
						}

						if (err.code == plupload.FILE_EXTENSION_ERROR) {
							alert(_("Error : รูปแบบไฟล์ไม่สนับสนุน : ") + file.name);
						}
