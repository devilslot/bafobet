...
//    PrintR($sql);
    ...
    //$country = $this->LoadMatch('travelcountry');
  ...
  public function OpenView(){
    $sql="
    SELECT
    al.*, ct.name_th AS leaguge_code
    FROM
    ".$this->table." al,  teams ct
    WHERE
    al.leaguge_code = ct.code AND
    al.code = '".$this->attr["code"]."'
    ";
//    echo $sql;
    $result['row'] = array();
  ...
  public function DelProduct($code){
    $sql="
    DELETE FROM
    teams
    WHERE
