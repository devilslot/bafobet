...
/*================================================*\
  ...
  me.ViewHeader=function(){
      ...
      {display:'ตำแหน่ง', name:'position_code', view:true, type:'cbo'},
      {display:'เว็บไซต์', name:'website'},
      {display:'วันหมดอายุ', name:'expire_date',cls:'dpk'}
      ]);
  };

  me.ViewOpenHeader=function(){
  	me.ViewOpenSetHeader([
      {display:'ตำแหน่ง', name:'position_code'},
      {display:'เว็บไซต์', name:'website'},
      {display:'วันหมดอายุ', name:'expire_date'},
  ...
  me.LoadCbo=function(){
    $.getJSON(me.url+'?mode=LoadCbo&mod='+me.mod+'&'+new Date().getTime(), {}, function(data){
      $.each(data.position, function(i, result) {
        ...
        $('<option>').attr('value', result.code).text(result.name).appendTo('#search_position_code');
      });
    });
  };
