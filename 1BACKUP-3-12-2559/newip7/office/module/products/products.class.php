...
//    PrintR($sql);
    ...
    $cat = $this->LoadMatch('categories');
    $subcat = $this->LoadMatch('categories_sub');
    foreach((array)$tmp as $i => $value){   
//      $result['row'][$i]['_id']=sprintf("%05d", $value['code']);
//      $result['row'][$i]['item'][]=$value[''];
      $value['filepic'] = Filepic($value['filepic'],50);
      $value['cat_code'] = $cat[$value['cat_code']]->name_th;
      $value['subcat_code'] = $cat[$value['subcat_code']]->name_th;
