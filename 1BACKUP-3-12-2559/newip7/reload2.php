...
$file='reload2.html'; // ชื่อไฟล์เว็บ
...
<table width="780" border="0" align="center" cellspacing="1" bgcolor="#202020" style="margin-top:5px;">
  <tr>
    <td height="42" colspan="9" background="bg-menu1.jpg" bgcolor="#004000"><div align="center" class="style5">
      <div align="left">&nbsp;&nbsp;<span class="style24">ผลบอลและทายสกอร์ ประจำวันที่ <?php  echo thai_date_fullmonth(time());?></span></div>
    </div></td>
  </tr>
  <tr>
  ...
  matchHDC
  WHERE
  Date >= '".$date." ".date('H:i:s')."' AND Date <= '".$date." 23:59:59' AND Status='0' AND Predict='Y'
   ORDER BY
			Status = '0' desc,
  ...
  while($row=mysql_fetch_assoc($query)){
   ...
   $new_time = strtotime($row['Date']);
				...
				$sqlN="SELECT code FROM playgame WHERE matchID = '".$row['MatchID']."' AND date_create LIKE '".$date."%';";
   ...
   <tr bgcolor="<?=$bgb?>">
    ...
    <td>
	  ...
	  if($new_time >  time()){ 
	  ?>
      <a  href="javascript:;" onclick="openplaygame(<?php echo $row['MatchID'];?>)"><img src="brt1.png" width="70" height="35" border="0" /></a>
      <?php }else{  ?>
      <a  href="javascript:;" onclick="openplaygame(<?php echo $row['MatchID'];?>)"><img src="off.png" width="70" height="35" border="0" /></a>
      <?php } ?>
     <?php if(Counttry($row['MatchID'])>0){ echo '<span class="badge yullow">'.Counttry($row['MatchID']).'</span >'; } ?>
       </td>
      <td>
      <a  href="javascript:;" onclick="opencomment(<?php echo $row['MatchID'];?>)"><img src="brt2.png" width="70" height="35" border="0" /></a>
