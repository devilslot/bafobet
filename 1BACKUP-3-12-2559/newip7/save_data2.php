...
foreach ($xml['category'] as $key => $value) {
	...
	$a++;
	echo '<pre>'.$a.':';print_r($value2[$a]);echo '</pre>';
		...
		if(trim($League) != 'Kosovo:Superliga'
		...
		$sqlckm="SELECT MatchID FROM matchHDC  WHERE MatchID = '".$id."' AND MatchID!='' ";
		...
		$datetimeout = strtotime ('+10hours',strtotime($date." ".$time));
		 ...
		 if($status!='' and strlen($status)=='5'){
		 	...
		 	$statusss = 2;
		 	mysql_query("update matchHDC set ScoreHomeHalfTime='".$local_team_goal."' , ScoreAwayHalfTime='".$visitor_team_goal."' where MatchID='".$id."' ");
		...
		if(empty($rowm['MatchID'])){
			...
			$sql="
				...
				ScoreAwayFulltime='".mysql_real_escape_string($visitorteamgoals)."',
				date_update = '".date('Y-m-d H:i:s')."'
				WHERE MatchID = '".$id."' AND Status!='-1' AND Predict='N'
			";
			//$query=mysql_query($sql) or die(mysql_error());
		}
	...
	$objQuery = mysql_query("select * from soccer_odds where id_match='".$id."' AND handicap_odd_handicap!='' AND handicap_odd_handicap2!='' order by code asc ");
	...
	echo 'sql2<pre>';print_r($sql);echo '</pre><hr>'; }else{ echo 'ห้าม<hr>'; }
