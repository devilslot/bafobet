                                    ...
                                    <hr />
										<form class="form-horizontal form-Right" action="" method="POST" onSubmit="sendWD(); return false;">
											 <div class="form-group" style="margin-top:25px;">
													...
													<div class="col-xs-5 col-md-5">
														<select id="wd_gameuser" name="wd_gameuser" onChange="" class="form-control w_form2 input-small" style="width:100%; height:30px;">
															...
															<option value="moneybag">บัตรเงินสด : ถุงเงิน</option>';
											 ...
											 <div class="form-group" style="margin-top:-5px;">
													...
													<div class="col-xs-5 col-md-5">
														<select id="tf_gameuser" name="tf_gameuser" onChange="trannewmember()" class="form-control w_form2 input-small" style="width:100%; height:30px;">
															<option value="">---กรุณาเลือก---</option>';
														
																$true_res = $db->GetAll("select * from gameusers where owner = '".$member['id']."' and provider != 'W88'");
																if(!empty($true_res)){
																foreach($true_res as $x){
																	$html .= '<option value="'.$x['gameuser'].'">'.strtoupper($x['gameuser']).'</option>';
																}
															...
															if($member['mactive']=='Y'){ $html .= '<option value="NEWUSER">เปิดบัญชีใหม่</option>'; }
														$html .= '</select>															
													</div>
											 </div>
											 ...
											 <div class="form-group" style="margin-top:-5px;">
													<div class="col-xs-offset-3 col-md-offset-3 col-xs-5 col-md-5">';
                                                      if($member['suped']!='sup'){ 
                                                   $html .= '<button type="submit" class="btn btn-primary"><Strong>ส่งข้อมูล</strong></button>'; }else{ $html .= 'ขณะนี้ท่านถูกระงับการทำรายการ กรุณาติดต่อแชท'; } 
