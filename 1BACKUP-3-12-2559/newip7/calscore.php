...
WHERE playgame.matchID = matchHDC.MatchID AND matchHDC.Status='-1' AND playgame.cal = '0' AND playgame.gametype='rank'
...
//echo "<br>sql2 : ".$sql2;
  ...
  *
  ...
  $num_rows = mysql_num_rows($querym);
	...
	if($num_rows==0){
	$sql4 = " INSERT INTO members_score (point,rank,memberID,months,years,username)VALUES('1000','".$scorepoint."','".$row['memberID']."','".$montplay."','".$yearplay."','".$row['user_create']."') ";
	mysql_query($sql4);
	}else{
	$sql4 = " UPDATE members_score SET rank = rank + '".$scorepoint."' ,date_update ='".date('Y-m-d H:i:s')."' WHERE memberID = '".$row['memberID']."' AND months='".$montplay."' AND years='".$yearplay."' ";
	mysql_query($sql4);
	}
//echo "<br>sql3 : ".$sql3;
...
if($scorepoint=='0'){ /*เสมอ*/ $status='3'; $totalpoint=$row['point']; }else 
if($scorepoint>='1'){ /*ได้*/  $status='1'; $totalpoint=(($row['point']*($scorepoint/2))+$row['point']);  }else 
if($scorepoint=='-1'){/* เสีย  0.5*/ $status='2'; $totalpoint=($row['point']/2);  }else 
if($scorepoint=='-2'){ /*เสีย 1 */ $status='2'; $totalpoint=0;  }
//echo "<br>".$scorepoint."<br>";	
