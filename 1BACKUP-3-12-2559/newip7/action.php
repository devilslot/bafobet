...
if(empty($password)){
	...
	if($ck2){
		...
		if($_SESSION[MEMBER]['LOGIN'] != 'ON'){
			mysql_query("INSERT INTO members (tel,username,password,enable,user_create,date_create,score,mainid)VALUES('".$phone."','".$username."','".md5($password)."','Y','".$username."','".date('Y-m-d H:i:s')."','1000','".$res['id']."') ",$conn);
		}else{
	...
	if($rowid){
		...
		$status['status'] = "1";
		$status['token'] = base64_encode(base64_encode($username));
        $status['session'] = base64_encode(base64_encode(md5($password)));
