...
if($_GET['token']!='' and $_GET['session']!='' ){
	$user = base64_decode(base64_decode($_GET['token']));
	$pass = base64_decode(base64_decode($_GET['session']));
  	$table = 'members';
  	$datenow = DateNow();
  	...
  	if(strcmp(md5($login['password']), $pass)==0){	
...
if($_GET['types']=='rank'){
        ...
        *
        FROM
        members_score
			where months='9' AND years='".date('Y')."'
        ORDER BY
        rank
        DESC
        LIMIT 3
		...
		while($rows=mysql_fetch_assoc($querys)){
			...
			if($rows['getbonusr']==0){
				...
				if($_SESSION[MEMBER]['DATA']['code']==$rows['memberID']){
					...
					$db->Execute("INSERT INTO paylogs (owner, name, username,phone,types,froms,tos,amount,status,added,added2,added3,added4,sure,toporder,q,personnel,ip,webroot,dep_bef,dep_aft) VALUES ('".$member['id']."','".$member['name']."','".$member['username']."','".$member['phone']."','coupon','CODE:MONTHFRD".$credit."B','MONEYBAG".$row['owner']."','".$credit."','confirmed','".$added."','".$added."','".$added."','".$added."','1','','รางวัลบาโฟรายเดือน ".$credit." บาท','','','bafobet','".$member['moneybag']."','".($member['moneybag']+$credit)."')");
...
if($member['active']=='Y' and !empty($_GET['freebonus'])){
		...
		if(strtoupper($allw)=='WWWWW'){
				...
				if($winlosss=='5' and $sumbo=='0'){ 
					...
					if(strtotime("+1 day", $member['mactive_date'])<=time()){ $bonusamount=200; }else{ $bonusamount=100; }
					
					$db->Execute("INSERT INTO paylogs (owner, name, username,phone,types,froms,tos,amount,status,added,added2,added3,added4,sure,toporder,q,personnel,ip,webroot,dep_bef,dep_aft) VALUES ('".$member['id']."','".$member['name']."','".$member['username']."','".$member['phone']."','coupon','CODE:WWWWW".$bonusamount."B','MONEYBAG".$row['owner']."','".$bonusamount."','confirmed','".$added."','".$added."','".$added."','".$added."','1','','รางวัลบาโฟW5ตัวใหญ่ ".$bonusamount." บาท','','','bafobet','".$member['moneybag']."','".($member['moneybag']+$bonusamount)."')");
					...
					if(strtotime("+1 day", $member['mactive_date'])<=time()){ $bonusamount=100; }else{ $bonusamount=50; }
					$db->Execute("INSERT INTO paylogs (owner, name, username,phone,types,froms,tos,amount,status,added,added2,added3,added4,sure,toporder,q,personnel,ip,webroot,dep_bef,dep_aft) VALUES ('".$member['id']."','".$member['name']."','".$member['username']."','".$member['phone']."','coupon','CODE:WwWwW".$bonusamount."B','MONEYBAG".$row['owner']."','".$bonusamount."','confirmed','".$added."','".$added."','".$added."','".$added."','1','','รางวัลบาโฟW5ตัวผสม ".$bonusamount." บาท','','','bafobet','".$member['moneybag']."','".($member['moneybag']+$bonusamount)."')");
					$db->Execute("UPDATE member SET moneybag = moneybag+$bonusamount where id= ".$member['id']." ");
				  	echo '<script> alert("ท่านได้รับโบนัส '.$bonusamount.' บาท!"); window.location="playbet.php"; </script>';
				}
...
<div style="width:700px; float:left;">
  ...
  <div style="width:290px; float:right;">
  	<button type="button" style="margin:0px auto 5px auto;" class="btn btn-warning btn-block dep-button" onclick="deptm()"><span class="glyphicon glyphicon-plus-sign"></span> ฝากบัตรเงินสดเข้าถุงเงิน</button><br />
    <button type="button" class="btn btn-warning btn-block dep-button" onclick="depbank()"><span class="glyphicon glyphicon-plus-sign"></span>  ฝากเงินธนาคารเข้าถุงเงิน</button>
  </div>
  ...
  <div style="width:1040px; margin:auto;  overflow:hidden; background:url(bg-body.png); min-height:500px;">
...
<table width="850" border="0" align="center" style="margin-bottom:15px;">
  <tr>
    ...
    <td><div class="gtb"></div>
        ...
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-primary" onclick="newusergames('Vegus168')" <?php echo $dcb;?>>เปิดไอดีเกมส์</button>
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-info" onclick="tranmoney('Vegus168')" <?php echo $dca;?>>โยกย้ายเงิน</button>
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-danger" onclick="wimoney('Vegus168')" <?php echo $dca;?>>ถอนเงิน</button></div></td>
  </tr>
  <tr>
    <td height="70" align="center">
    	<div class="buttonbet-pv">
        	<img src="images/logogclub.png" width="160" style="margin-top:15px;" />
    ...
    <td><div class="gtb"></div>
        ...
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-primary" onclick="newusergames('GCLUB')" <?php echo $dcb;?>>เปิดไอดีเกมส์</button>
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-info" onclick="tranmoney('GCLUB')" <?php echo $dca;?>>โยกย้ายเงิน</button>
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-danger" onclick="wimoney('GCLUB')" <?php echo $dca;?>>ถอนเงิน</button></div></td>
  </tr>
  <tr>
    <td height="70" align="center">
    	<div class="buttonbet-pv">
        	<img src="images/logosbobet.png" width="160"  style="margin-top:15px;" />
        </div>
    </td>
  <td><div class="gtb"></div>
    	<div class="buttonbet">
        ...
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-primary" onclick="newusergames('SBOBET')" <?php echo $dcb;?>>เปิดไอดีเกมส์</button>
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-info" onclick="tranmoney('SBOBET')" <?php echo $dca;?>>โยกย้ายเงิน</button>
        <button type="button" style="margin:14px 20px 0px auto;" class="btn btn-danger" onclick="wimoney('SBOBET')" <?php echo $dca;?>>ถอนเงิน</button></div></td>
  </tr>
</table>
...
<div id="myModalBet" class="modal fade" role="dialog">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content" style="background-color:#222222; color:#FFFFFF; padding:0;">
      ...
      <div id="myModalBetContent" class="modal-body" style="overflow:visible;" >
      </div>
