...
me.ViewHeader=function(){
	me.ViewSetHeader([
		{display:'ตำบล', name:'name_th'},
		{display:'อำเภอ', name:'amphur_code', type:'cbo', option:false},
...
me.LoadCbo=function(){
  $.getJSON(me.url+'?mode=LoadCbo&mod='+me.mod+'&'+new Date().getTime(), {}, function(data){
    $.each(data.amphur, function(i, result) {
      $('<option>').attr('value', result.code).text(result.name).appendTo('#amphur_code');
      $('<option>').attr('value', result.code).text(result.name).appendTo('#search_amphur_code');
    });
