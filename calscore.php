...
WHERE playgame.matchID = matchHDC.MatchID AND (matchHDC.Status='-1' OR matchHDC.Status='Und')  AND playgame.cal = '0' 
...
if($calctype=='b'){
	...
	$date_year=date('Y', $date_create);
	$date_mon=date('m', $date_create);
	$date_day=date('d', $date_create);
	$date_hour=date('H', $date_create);
	
	if($date_day=='01' and $date_hour<'12'){
		$date_mon=date("m", strtotime("-1 day",strtotime($row['date_create'])));
		$date_year=date("Y", strtotime("-1 day",strtotime($row['date_create'])));
	}
...
if(($row['gametype']=='even' or $row['gametype']=='rank') and $row['Status']=='Und'){
	$sql2 = " UPDATE playgame SET winloss = '0.00',status = 'Can', cal = 1 WHERE code = '".$row['code']."' ";
	mysql_query($sql2);
	echo "<br>UND-even<br>";
}


if($row['gametype']=='point' and $row['Status']=='Und'){
	$sql2 = " UPDATE playgame SET winloss = '0.00',status = 'Can', cal = 1 WHERE code = '".$row['code']."' ";
	mysql_query($sql2);
	echo "<br>UND-Point<br>";
	
	if(date('m')==$date_mon or $date_acpass=='Y'){
		$sql3 = " UPDATE members SET score = score + '".$row['point']."' WHERE code = '".$row['memberID']."' ";
		mysql_query($sql3);
	}
}
...
if(($row['gametype']=='rank' or $row['gametype']=='even') and $row['Status']!='Und' ){
...
//echo "<br>sql3 : ".$sql3;
  ...
  $num_rows = mysql_num_rows($querym);
	...
	$sqlsr="SELECT score,rank FROM members WHERE code = '".$row['memberID']."' ;";
