...
tmt_globalRules.DEFAULT_DATE_PATTERN = "YYYY-MM-DD";
tmt_globalRules.datepattern = function(fieldNode){
	var datObj = tmt_dateStrToObj(fieldNode.value, fieldNode.getAttribute("tmt:datepattern"));
	if(datObj){
		return true;
	}
	return false;
}
tmt_globalRules.maxdate = function(fieldNode){
	// Default pattern
	var pattern = tmt_globalRules.DEFAULT_DATE_PATTERN;
	if(fieldNode.getAttribute("tmt:datepattern")){
		pattern = fieldNode.getAttribute("tmt:datepattern");
	}
	var valueDate = tmt_dateStrToObj(fieldNode.value, pattern);
	var maxDate = tmt_dateStrToObj(fieldNode.getAttribute("tmt:maxdate"), pattern);
	if(valueDate && maxDate){
		return valueDate <= maxDate;
	}
	return false;
}
tmt_globalRules.mindate = function(fieldNode){
	// Default pattern
	var pattern = tmt_globalRules.DEFAULT_DATE_PATTERN;
	if(fieldNode.getAttribute("tmt:datepattern")){
		pattern = fieldNode.getAttribute("tmt:datepattern");
	...
	var valueDate = tmt_dateStrToObj(fieldNode.value, pattern);	
	var minDate = tmt_dateStrToObj(fieldNode.getAttribute("tmt:mindate"), pattern);
...
function tmt_dateStrToObj(dateStr, datePattern){
	var globalObj = tmt_globalDatePatterns[datePattern];
	if(globalObj){
