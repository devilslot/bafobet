	...
	$ckp = $db->GetRow("select * from coupon where couponcode = '$coupon'");
		...
		if(($ckp['exp']!='0')and($ckp['exp']<$tim)){
			...
			if($ckp['type']=='percent'){
				...
				$couponamount = ($amount*$ckp['amount']/100);
				$couponamountorg = ($amount*$ckp['amount']/100);
				
				if($member['bonus_date'] == date('d')){
				
					$couponamount = $couponamount+$member['bonus_amount'];
					
				}else{
					$db->Execute("UPDATE member SET bonus_date = '".date('d')."', bonus_amount=0 ,added='".time()."' where id= ".$member['id']." ");
					$member['bonus_amount']=0;
					$couponamount = $couponamount+$member['bonus_amount'];
				}
				
				if($couponamount>=1000){ $couponamount=(1000-$member['bonus_amount']); }else{ $couponamount=$couponamountorg; }
				...
				$db->Execute("INSERT INTO couponlogs (coupon,couponcode,amount,befam,aftam,type,owner,name,added,status) VALUES ('".$ckp['id']."','".$coupon."','".$ckp['amount']."','".$amount."','".($amount+$couponamount)."','".$ckp['type']."','".$member['id']."','".$member['name']."','".time()."','pending')");
				
				
				
				$amountt = $couponamount;
			...
			$db->Execute("UPDATE member SET bonus_date = '".date('d')."', bonus_amount=bonus_amount+".$amountt." where id= ".$member['id']." ");
		}*/
		
		if($coupon!='' and $bank =='TrueWallet-TrueWallet'){
$status['status'] = "0";
			$status['msg'] = "TrueWallet ไม่เข้าร่วมรายการ คูปอง ";
			echo json_encode($status);
			exit();
}
		if($coupon!=''){
		...
		if(($coupon=='FREEBONUSTBSBET10P') or ($coupon=='FREEBONUSFIFA5510P') or ($coupon=='FREEBONUSVEGUS16810P') or ($coupon=='FREEBONUSGCLUB10P') or ($coupon=='FREEBONUSSBOBET10P') or ($coupon=='FREEBONUS10P')or ($coupon=='FREEBONUSREGIS')){
		if($coupon=='FREEBONUS10P'){ $perc=10; }else{ $perc=10; }
				$couponamount = ($amount*$perc/100);
				$couponamountorg = ($amount*$perc/100);
				...
				if($couponamount>=100){ $couponamount=(100-$member['bonus_amount']); }else{ $couponamount=$couponamountorg; }
				...
				$db->Execute("INSERT INTO couponlogs (coupon,couponcode,amount,befam,aftam,type,owner,name,added,status) VALUES ('','".$coupon."','$perc','".$amount."','".($amount+$couponamount)."','%','".$member['id']."','".$member['name']."','".time()."','pending')");
				...
				$qsys = ' | คูปองC:'.$coupon.' โบนัส'.$perc.'% มูลค่า '.$amountt.' บาท';
	...
	$rowid = $db->Execute("INSERT INTO deposit (owner,name,gameuser,username,bank,added,times,amount,bonus,q,qcoupon,coupon,ip,webroot) VALUES ('$owner','$name','$gameuser','$username','$bank','$added','$timestampp','$amount','$amountt','".$q."','".$qsys."','".$coip."','$ip','$webroot')");
	if($rowid){
		...
		$status['status'] = "1";
		$status['rid'] = $rowid;
		$status['msg'] = "".$alrt." รายการฝากเงินได้รับการบันทึกเรียบร้อย โปรดรอสักครู่ ......... ทีมงานกำลังตรวจสอบราย";
		echo json_encode($status);
		exit();
	}
...
if(isset($_POST['opennewgames'])){
	...
	$tranfers = trim($_POST['tranfers']);
		...
		if($rowid){
			...
			$status['status'] = "1";
			$status['rid'] = $rowid;
			$status['msg'] = "รายการเปิด ID เกมส์ได้รับการบันทึกเรียบร้อย จะได้รับ id pass ทาง sms ภายใน 30 นาที";
			echo json_encode($status);
			exit();
		}
	//}
