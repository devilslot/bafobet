...
//AND FT!='-1'
$Query = mysql_query("select * from soccer_match where unix_timestamp(date_match) >= '".strtotime("-1 day", strtotime(date('Y-m-d')))."'  and id_match!='' ");
...
//echo '<hr>'.$i.': '.$Result['id_match'].'';
		...
		if(empty($rowm['HomeName'])){
								...
								WHERE MatchID = '".$Result['id_match']."'  AND Predict='N' AND Status!='-1'
								";
