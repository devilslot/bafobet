...
$user=array("Season","Yummy","Absolut","hongthong","madoka","ayukawa","KakJung","llnJllSJllSJ","Syona","Eflex","sunner","apnpete");
...
for($i=0;$i<=count($rand_key_user);$i++){
	if($user[$rand_key_user[$i]]!=''){
		...
		if($memberdata['getpoint']!=$dgpt){
			...
			$datepostgp = date('Y-m-d H:i:s',strtotime("-20 minutes"));
			$sqlup2="insert into playgame (matchID,point,winloss,memberID,status,cal,gametype,user_create,user_update,date_create,date_update)value('GETPOINT','500','1.00','".$memberdata['code']."','1','1','point','".$memberdata['username']."','".$memberdata['username']."','".$datepostgp."','".$datepostgp."') ";
		...
		while($row=mysql_fetch_assoc($query))
		{
			...
			$bet = rand(1,2);
				...
				if($looppoint>=$fortime and $num4>=5 )
				{
					break;
				}
