...
$file='tryscore-today.html'; // ชื่อไฟล์เว็บ
...
<?php
  $sql="SELECT * FROM matchHDC WHERE Date >= '".$date." ".date('H:i:s')."' AND Date <= '".$date." 23:59:59' AND Status='0' AND Predict='Y' ORDER BY Status = '0' desc, Date asc";
  // echo $sql;
  $query=mysql_query($sql);
  $tmpl ='';
  while($row=mysql_fetch_assoc($query)){
  	$loopp++;
    if($loopp % 2 == 0){ $bgb ="#282828"; }else{ $bgb ="#1D1D1D"; }
